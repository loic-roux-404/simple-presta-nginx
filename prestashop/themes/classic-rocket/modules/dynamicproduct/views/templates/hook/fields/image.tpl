...
<div class="dp_uploader_div dp_uploader_div_{$field->id|intval}" data-id_field="{$field->id|intval}">
    <input id="dp_{$field->name|escape:'htmlall':'UTF-8'}"
           ...
           class="dp_entry dp_image {if $field->settings->required && $dp_config->required}dp_required{/if}"
           type="hidden"
           data-name="{$field->name|escape:'htmlall':'UTF-8'}"
           data-label="{$field->label|escape:'htmlall':'UTF-8'}"
           value="">
    <a class="btn btn-default btn-primary button button-medium dp_file_uploader" href="#" title="{l s='Upload an image from your computer' mod='dynamicproduct'}">
        <span>
            {l s='Upload' mod='dynamicproduct'}<i class="icon-image right"></i>
        </span>
        <input type="file">
    </a>
    <div class="dp_thumb_container">
        <a href="#" class="dp_file_remove" title="{l s='Remove this image' mod='dynamicproduct'}"></a>
        <a target="_blank" class="dp_file_thumb" title="{l s='Click to enlarge' mod='dynamicproduct'}"></a>
    </div>
    {include file="../tooltip/image-tooltip.tpl"}
